...
MENU_ID = 0x80479D30
AP_CHAR_UNLOCKS = 0x80001810
SECRET_CHAR_ADDRESS = 0x8045BF28
SECRET_STAGE_ADDRESS = 0x8045BF2A
AP_EVENT_COUNTER = 0x80001817
LOTTERY_POOL_UPGRADES = 0x80001818
...
class SSBMClient(CommonContext):
    ...
    def __init__(self, server_address, password):
        ...
        self.other_events_complete = False
    ...
    def make_gui(self):
        ui = super().make_gui()
        ui.base_title = f"Super Smash Bros. Melee {apname} Client"
        return ui
    ...
    async def check_goal_data(self):
        ...
        logger.info("GOALS: ")
        if "Giga Bowser" in self.active_goals:
        ...
        if "Crazy Hand" in self.active_goals:
        ...
        if "Event 51" in self.active_goals:
        ...
        if "Event 50" in self.active_goals:
            if self.event_50_complete:
                message = "COMPLETE"
            else:
                message = "INCOMPLETE"
            logger.info(f"Complete Event 50: {message}")
        ...
        if all([
            ...
            self.other_events_complete,
            self.event_51_complete,
            self.event_50_complete,
                ...
                if ctx.slot:
                    ...
                    if not ctx.auth:
                        ...
                        if ctx.auth == "SuperSmashBros0110290334":  # Vanilla save code
                            ctx.auth = None
                            ctx.awaiting_rom = False
                            logger.info(f"{apname} patch not loaded. Please load a Melee {apname} Riivolution patch and try again.")
                            ctx.dolphin_status = CONNECTION_REFUSED_GAME_STATUS
                            dme.un_hook()
                            await asyncio.sleep(5)
                            continue
